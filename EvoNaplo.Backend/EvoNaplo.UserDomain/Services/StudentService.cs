...
namespace EvoNaplo.UserDomain.Services
{
    public class StudentService
    {
        ...
        internal EmailViewModel EmailExists(string email)
        {
            if (_userRepository.GetAll().FirstOrDefault(u => u.Email == email) != null)
            {
                return new EmailViewModel { Email = email , Exists = true};
            }
            else
            {
                return new EmailViewModel { Email = String.Empty , Exists = false};
            }
        }
